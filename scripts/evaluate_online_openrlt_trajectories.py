...
def evaluate_trajectory(
    ...
    fwd = traj.get("forward_inputs", {})
    curr = traj["curr_obs"]
    ref_action = fwd.get("ref_action", curr["ref_action"])[:, 0].reshape(-1, 10, 6)
    states = curr["states"][:, 0].reshape(-1, 14)
    visual = fwd.get("visual_latent", curr["visual_latent"])[:, 0]
    rewards_full = traj["rewards"][:, 0].float()
    dones_full = traj["dones"][:, 0].bool()
    valid_mask = fwd.get("chunk_valid_mask")
    if valid_mask is None:
        valid = torch.ones(ref_action.shape[0], 10, dtype=torch.bool)
    else:
    ...
    if "executed_env_action_absolute" in fwd:
        pt_abs_full_tensor = (
            fwd["executed_env_action_absolute"][:, 0].reshape(n, 10, 14).float()
        )
        ...
        actions = torch.cat(converted_actions, dim=0).reshape(n, 10, 6)
    elif "env_action_absolute" in fwd:
        pt_abs_full_tensor = fwd["env_action_absolute"][:, 0].reshape(n, 10, 14).float()
        ...
        actions = fwd.get("executed_action", traj["actions"])[:, 0].reshape(-1, 10, 6)
        pt_abs_full_tensor = None
        data_action_source = "executed_action"

    stored_action = fwd.get("executed_action", traj["actions"])[:, 0].reshape(n, 10, 6)
    ...
    if pt_abs_full_tensor is not None:
        ...
        pt_abs_full = np.zeros((n, 10, 14), dtype=np.float32)
    ...
    mc = compute_chunk_mc_returns(
        reward_chunk, done_chunk, gamma=gamma, action_horizon=10
    )
...
def main() -> int:
    ...
    policy = build_policy(checkpoint, args.rl_token_path, args.norm_stats_path, device)
